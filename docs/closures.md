...
const add_to_100 = create_adder(100) // 100 is a constant value
...
fn main() {
  ...
  const add_to_user_input = create_adder(input)

  const my_constant = 23 // Constant value
...
```rust
let list = [1, 2, 3]
let new_list = reduce!create_adder(list)
```