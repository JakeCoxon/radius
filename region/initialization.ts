...
export class RegionInitializationCheckingPass {
  ...
  nextVisitInstruction: InstructionId | null = null
  ...
  interpret() {
    ...
    worklist.fixedPoint((regionId) => {
      ...
      compilerAssert(inputStates.every(x => x), `Predecessor state not found`, { regionId, predecessors });
      compilerAssert(inputStates[0]?.output, `Predecessor output state not found`, { regionId, predecessors, inputStates });
  ...
  executeRegion(regionId: RegionId, inputState: InterpreterState) {
    ...
    this.blockStates.set(regionId, { input: cloneState(inputState), output: cloneState(this.state) });
  }

  executeInstruction(instr: IRInstruction): void {
    if (this.debugLog) {
      if (!(instr instanceof CommentInstruction)) {
        console.log(`${this.instrId}.`, formatInstruction(instr));
      }
    }
