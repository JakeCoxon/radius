...
const createConstructor = (vm: Vm, constructor: ArrayConstructorCompiler) => {
  const constructorArgs = [new NamedArgAst(IntType, vm.location, "initial_length", new NumberAst(IntType, vm.location, 0))]
  return (
...
const appendValuePartialFn = (() => {
  ...
  const let_ = new ParseLet(token, LetType.Alias, iden, null, exprQuote)
...
const compileListConstructorForExpr = (out: BytecodeWriter, node: ParseForExpr, listConstructorIden: ParseFreshIden) => {
  ...
  const let_ = new ParseLet(node.token, LetType.Var, iden, null, exprQuote)
...
const compileListConstructorExpand = (out: BytecodeWriter, node: ParseExpand, listConstructorIden: ParseFreshIden) => {
  ...
  const let_ = new ParseLet(node.token, LetType.Alias, iden, null, exprQuote)
  const call = callV(node.token, arrayConstructorTypeCheck, [listConstructorIden, iden], [])
  const call2 = callV(node.token, arrayConstructorCreateAppend, [listConstructorIden, iden], [])
...
const letIn = (token: Token, node: ParseNode, f: (iden: ParseFreshIden) => ParseNode[]) => {
  ...
  const let_ = new ParseLet(token, LetType.Alias, iden, null, node)
...
const createIteratorSliceLoopPartialFn = (() => {
  ...
  const letStartNode = metaIf(token, startIden, 
    new ParseLet(token, LetType.Alias, new ParseFreshIden(token, new FreshBindingToken('start')), null, startIden), null)
  const letEndNode = metaIf(token, endIden,
    new ParseLet(token, LetType.Alias, new ParseFreshIden(token, new FreshBindingToken('end')), null, endIden), null)
  const letStepNode = metaIf(token, stepIden, 
    new ParseLet(token, LetType.Alias, new ParseFreshIden(token, new FreshBindingToken('step')), null, stepIden), null)
  const letIndexNode = new ParseLet(token, LetType.Alias, indexIdentifier, null, new ParseNumber(createAnonymousToken('0')))
  ...
  const consumeCall = new ParseLet(token, LetType.Alias, consumedValue, null, new ParseCall(createAnonymousToken(''), consumeParam, [], []))
...
const compileExpansionToParseNode = (out: BytecodeWriter, expansion: ExpansionCompilerState, node: ParseNode) => {
  ...
  if (expansion.fold) {
    lets.push(new ParseLet(node.token, LetType.Alias, expansion.fold.iden, null, expansion.fold.initial))
  ...
  const metaLet = new ParseLet(node.token, LetType.Alias, expansion.metaResultIden, null, new ParseQuote(node.token, block))
...
const zipHelper = (debugName: string, zips: ZipIterator[], result: (stmts: ParseNode[]) => ParseNode, index: number = 0, letStmts: ParseNode[] = []): ParseNode => {
  ...
  const let_ = new ParseLet(token, LetType.Alias, elemIden, null, new ParseCall(token, consumeParam1, [], []))
