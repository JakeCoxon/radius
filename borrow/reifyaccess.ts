...
import { AccessInstruction, Capability, FunctionBlock, IRInstruction, InstructionId, LoadFromAddressInstruction, MoveInstruction, compilerAssert, createUsageMap } from "./defs";
  ...
  reifyAccesses() {
    ...
    while (worklist.length > 0) {
      ...
      const { min, max } = usageList.reduce((acc, usage) => {
        ...
        compilerAssert(reqs.length > 0, 'No capabilities found')
        let x = Math.min(...reqs.map(x => CapabilityRanking.indexOf(x)))
        return { min: Math.max(acc.min, x), max: Math.max(acc.max, x) }
      }, { min: 0, max: 0 })

      if (min === max) {
        const cap = CapabilityRanking[min]
        compilerAssert(instr.capabilities.includes(cap), `Capability ${cap} not found in ${instr.capabilities}`)
...
const capabilitiesOfInstr = (instr: IRInstruction, operandIndex: number) => {
  if (instr instanceof AccessInstruction) {
    ...
    return operandIndex === 0 ? /* target */ [Capability.Inout] : /* source */ [Capability.Sink]
